...
class CutlistRenderer {
  constructor(canvas) {
    ...
    this.instance = null; // current SheetInstance
    this.kerf = 3;
  ...
  zoomIn() {
    this._zoomAround(this.canvas.width / 2, this.canvas.height / 2, 1.25);
  }
  zoomOut() {
    this._zoomAround(this.canvas.width / 2, this.canvas.height / 2, 0.8);
  }
  ...
  render() {
    ...
    this._drawSheet();
    if (this.snapEnabled) this._drawGrid();
  ...
  _drawLabel(p, screenW, screenH) {
    ...
    const maxFs = 14;
    const minFs = 8;
    const fs = Math.max(minFs, Math.min(maxFs, screenW / 8)) / this.zoom;
    ...
    ctx.fillText(label, cx, cy - fs * 0.6);
  ...
  _drawGrid() {
    ...
    ctx.beginPath();
    for (let x = g; x < sw; x += g) { ctx.moveTo(x, 0); ctx.lineTo(x, sh); }
    for (let y = g; y < sh; y += g) { ctx.moveTo(0, y); ctx.lineTo(sw, y); }
    ctx.stroke();
    ctx.setLineDash([]);
  }

  _drawKerfGap() {
    // Subtle crosshatch on kerf strips – skip when zoomed out
    if (this.zoom < 0.3 || this.kerf <= 0) return;
    // (visual indication only for the canvas overlay – skipped for simplicity)
  }
  ...
  _hitTest(cx, cy) {
    ...
    const ps = [...this.instance.placements].reverse();
    return (
      ps.find(
        (p) =>
  ...
  _bindEvents() {
    ...
    c.addEventListener("dblclick", (e) => {
      if (this.onDblClick) this.onDblClick(this._hitTest(this._cx(e), this._cy(e)));
    });
    window.addEventListener("keydown", e => { if (e.key === "Shift") this.shiftDown = true; });
    window.addEventListener("keyup",   e => { if (e.key === "Shift") { this.shiftDown = false; this.render(); } });
  }
  ...
  _onUp(e) {
    if (this.drag) {
      ...
      if (this.onMoved) this.onMoved(p, success);
      this.drag = null;
      this.render();
    }
    if (this.isPanning) {
  ...
  _hasOverlap(moved) {
    return this._getOverlaps(moved).length > 0;
  }

  _trySnapToValid(p, sw, sh) {
    const k = this.kerf;
    const threshold = 5 * k;
    const overlaps = this._getOverlaps(p);
    const origX = p.x, origY = p.y;
    let bestDist = Infinity, bestX = origX, bestY = origY;
    ...
    for (const o of overlaps) {
      ...
      for (const c of candidates) {
        ...
        p.x = nx; p.y = ny;
        if (!this._hasOverlap(p)) { bestDist = dist; bestX = nx; bestY = ny; }
      }
    }

    p.x = origX; p.y = origY;
    if (bestDist <= threshold) { p.x = bestX; p.y = bestY; return true; }
    return false;
  }
