...
const State = {
    settings: { kerf: 3, allowRotation: true },
    sheets: [],
    parts: [],
    results: null,
    ui: { activeResultIdx: 0, activeInstanceIdx: 0 },
};

let _sheetEditId = null;
...
function nextSheetColor() { return SHEET_COLORS[_sheetColorIdx++ % SHEET_COLORS.length]; }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    renderer.onZoomChange = z => { document.getElementById('stat-zoom').textContent = `${Math.round(z * 100)}%`; };
    renderer.onSelectionChange = pl => updateSelectionInfo(pl);
    renderer.onMoved = (pl, ok) => {
        showToast(ok ? 'Del flyttet.' : 'Overlapp – posisjon tilbakestilt.', ok ? 'info' : 'warn');
        updateStats();
    };
    bindTopBarEvents();
    bindSettingsEvents();
...
function bindTopBarEvents() {
    ...
    document.getElementById('btn-import-demo').addEventListener('click', () => { loadDemo(); showToast('Demo-data lastet.', 'info'); });
}
...
async function handleCSVImport(e) {
    ...
    e.target.value = '';
    try {
        const { sheets, parts } = csvParser.parse(await file.text());
        if (sheets.length) { sheets.forEach(s => s.color = s.color || nextSheetColor()); State.sheets.push(...sheets); }
        if (parts.length) State.parts.push(...parts);
        renderSidebar();
        showToast(`Importerte ${parts.length} deler${sheets.length ? ' og ' + sheets.length + ' plater' : ''}.`, 'success');
    } catch (err) { showToast('CSV-feil: ' + err.message, 'error'); }
}

function bindSettingsEvents() {
    document.getElementById('kerf').addEventListener('input', e => { State.settings.kerf = parseFloat(e.target.value) || 0; });
    document.getElementById('allow-rotation').addEventListener('change', e => { State.settings.allowRotation = e.target.checked; });
}
...
function runOptimizer() {
    ...
    setTimeout(() => {
        try {
            ...
            State.ui.activeResultIdx = State.ui.activeInstanceIdx = 0;
            renderResults();
            showToast(`Ferdig! ${State.results.totalSheets} plater, ${State.results.totalEfficiency.toFixed(1)}% effektivitet.`, 'success');
        } catch (err) { showToast('Optimeringsfeil: ' + err.message, 'error'); }
        finally { btn.disabled = false; btn.querySelector('.btn-label').textContent = 'Optimaliser'; }
    }, 30);
}

function exportPDF() {
    if (!State.results) { showToast('Kjør optimaliseringen først.', 'warn'); return; }
    try { pdfExp.export(State.results, State.settings); showToast('PDF eksportert.', 'success'); }
    catch (err) { showToast('PDF-feil: ' + err.message, 'error'); }
}
...
function renderResults() {
    ...
    tabBar.innerHTML = '';
    const allInstances = [];
    for (const sr of res.sheetResults) for (const inst of sr.instances) allInstances.push({ sr, inst });
    allInstances.forEach(({ sr, inst }, idx) => {
        const tab = document.createElement('button');
        tab.className = 'sheet-tab' + (idx === 0 ? ' active' : '');
        tab.innerHTML = `<span>${sr.sheetDef.name} #${inst.index + 1}</span><span class="tab-eff">${inst.efficiency.toFixed(0)}%</span>`;
    ...
    if (allInstances.length) showInstance(allInstances[0].inst);
    updateStats();
    if (res.unplaced.length) showToast(`${res.unplaced.length} del(er) fikk ikke plass – sjekk platedimensjoner.`, 'warn');
}

function showInstance(inst) { renderer.setInstance(inst, State.settings.kerf); updateStats(); }

function updateStats() {
    const inst = renderer.instance; if (!inst) return;
    document.getElementById('stat-current-sheet').textContent = `${inst.sheetDef.name} #${inst.index + 1}`;
    document.getElementById('stat-efficiency').textContent   = `${inst.efficiency.toFixed(1)}%`;
    document.getElementById('stat-parts-placed').textContent = inst.placements.length;
    document.getElementById('stat-unplaced').textContent     = State.results ? State.results.unplaced.length : '-';
}

function updateSelectionInfo(pl) { /* future detail panel */ }
...
function saveSheet() {
    ...
    if (!name || !width || !height || qty < 1) { showToast('Fyll ut alle felt korrekt.', 'warn'); return; }
    if (_sheetEditId) Object.assign(State.sheets.find(s => s.id === _sheetEditId), { name, width, height, qty, color });
    else State.sheets.push({ id: uid('s'), name, width, height, qty, color });
    closeSheetModal(); renderSidebar(); showToast(`Plate "${name}" lagret.`, 'success');
}
...
function bindPartEvents() {
    ...
    document.getElementById('part-modal').addEventListener('click', e => { if (e.target === e.currentTarget) closePartModal(); });
}
...
function openPartModal(id) {
    ...
    for (const s of State.sheets) { const o = document.createElement('option'); o.value = s.name; o.textContent = s.name; sel.appendChild(o); }
    if (id) {
        const p = State.parts.find(p => p.id === id);
        document.getElementById('part-modal-title').textContent = 'Rediger del';
        ...
        document.getElementById('part-modal-title').textContent = 'Legg til del';
...
function savePart() {
    ...
    if (!name || !width || !height || qty < 1) { showToast('Fyll ut alle felt korrekt.', 'warn'); return; }
    if (_partEditId) Object.assign(State.parts.find(p => p.id === _partEditId), { name, width, height, qty, material });
    else State.parts.push({ id: uid('p'), name, width, height, qty, material });
    closePartModal(); renderSidebar(); showToast(`Del "${name}" lagret.`, 'success');
}

function renderSidebar() { renderSheetList(); renderPartList(); }

function renderSheetList() {
    const el = document.getElementById('sheets-list');
    if (!State.sheets.length) { el.innerHTML = '<p class="empty-hint">Ingen plater lagt til ennå.</p>'; return; }
    el.innerHTML = State.sheets.map(s => `
        <div class="list-item" data-id="${s.id}">
            ...
            <div class="item-actions">
                <button class="icon-btn" onclick="openSheetModal('${s.id}')" title="Rediger"><span class="material-icons">edit</span></button>
                <button class="icon-btn danger" onclick="deleteSheet('${s.id}')" title="Slett"><span class="material-icons">delete</span></button>
            </div>
        </div>`).join('');
}
...
function renderPartList() {
    ...
    if (!State.parts.length) { el.innerHTML = '<p class="empty-hint">Ingen deler lagt til ennå.</p>'; return; }
    const countEl = document.getElementById('parts-count');
    if (countEl) countEl.textContent = State.parts.reduce((s, p) => s + (p.qty || 1), 0);
    el.innerHTML = State.parts.map(p => `
        <div class="list-item" data-id="${p.id}">
            ...
            <div class="item-actions">
                <button class="icon-btn" onclick="openPartModal('${p.id}')" title="Rediger"><span class="material-icons">edit</span></button>
                <button class="icon-btn danger" onclick="deletePart('${p.id}')" title="Slett"><span class="material-icons">delete</span></button>
            </div>
        </div>`).join('');
}

function deleteSheet(id) { if (!confirm('Slett denne platen?')) return; State.sheets = State.sheets.filter(s => s.id !== id); renderSidebar(); }
function deletePart(id)  { State.parts  = State.parts.filter(p => p.id !== id);  renderSidebar(); }
